...
// Step outputs a step indicator
func (w *ConsoleWriter) Step(number int, message string) {
	if w.level <= LevelInfo {
		prefix := w.colorize(ColorCyan, "→")
...
func (w *ConsoleWriter) writeWithPrefix(prefix string, format string, args ...interface{}) {
	w.mu.Lock()
	defer w.mu.Unlock()
...
func (w *ConsoleWriter) colorize(color string, text string) string {
	if w.noColor {
		return text
	}
