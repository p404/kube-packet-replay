...
// WaitForEphemeralContainerRunning polls the pod until the named ephemeral container
// reaches the "Running" state or the timeout is reached.
func (c *Client) WaitForEphemeralContainerRunning(namespace, podName, containerName string, timeout time.Duration) error {
		...
		for i := range pod.Status.EphemeralContainerStatuses {
			...
			if cs.Name == containerName {
				...
				if cs.State.Terminated != nil {
					return fmt.Errorf("ephemeral container %s terminated with exit code %d: %s",
						containerName, cs.State.Terminated.ExitCode, cs.State.Terminated.Reason)
				}
			}
		}
		time.Sleep(500 * time.Millisecond)
	}
	return fmt.Errorf("timeout waiting for ephemeral container %s to start", containerName)
}
...
func (c *Client) CreateDebugContainerWithKubectl(namespace, podName, containerName, debugImage string, command []string, debugContainerName string) error {
	...
	// Add profile, quiet flag, and command separator
	args = append(args, "--profile", "netadmin", "--quiet", "--")
...
func (c *Client) ExecInContainer(namespace, podName, containerName string, command []string) (string, error) {
	...
	if containerName != "" {
		...
		kubectlArgs := []string{"exec", "-n", namespace, "-c", containerName, podName, "--"}
		...
		// Return the kubectl error directly instead of falling back to the API method
		// which may produce confusing "unable to upgrade connection" errors
		return "", fmt.Errorf("failed to execute command: %v, stderr: %s", err, stderr.String())
	}
