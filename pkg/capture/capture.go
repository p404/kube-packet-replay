...
func CapturePackets(client *k8s.Client, namespace, podName, containerName, filterExpr, outputFile string, duration time.Duration, verbose bool) error {
	...
	out.Print("\n%s %s\n\n", 
		out.(*outpkg.ConsoleWriter).FormatBold("KUBE-PACKET-REPLAY CAPTURE STARTED AT:"), 
	...
	out.Print("  %s: %s\n", 
		out.(*outpkg.ConsoleWriter).Colorize(outpkg.ColorBlue, "Target Pod"),
		out.(*outpkg.ConsoleWriter).FormatBold(podName))
	
	// Display container name if specified
	if containerName != "" {
		out.Print("  %s: %s\n", 
			out.(*outpkg.ConsoleWriter).Colorize(outpkg.ColorBlue, "Container"),
	...
	out.Print("  Creating debug container %s...\n", out.(*outpkg.ConsoleWriter).FormatBold(debugContainerName))
	...
	out.Print("  Packet capture downloaded: %s (%s)\n", 
		out.(*outpkg.ConsoleWriter).FormatBold(outputFile),
		out.(*outpkg.ConsoleWriter).Colorize(outpkg.ColorGreen, formatBytes(int64(bytes))))
	out.Success("  Done")
	return nil
}
